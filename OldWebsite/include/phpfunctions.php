...
function CustomExpression($value, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="Product Inputs" && $field=="ProdGrpCode")
	{
		$value = $data["ProdGrpCode"]." ".$data["ProdNo"];
		return $value;
	}	
				if($table=="Product Inputs" && $field=="ProdEngName")
	{
		$value = $data["ProdEngName"]." ".$data["EngModel"];
		return $value;
	}	
				if($table=="Product Inputs" && $field=="ProdThaiName")
	{
		$value = $data["ProdThaiName"]." ".$data["ThaiModel"];
		return $value;
	}	
				if($table=="Product Inputs" && $field=="CostType")
	{
		;
		return $value;
	}	
				if($table=="Product Cost Breakdown" && $field=="CostType")
	{
		if ($value == "Capital Costs")
	$value = NULL;;
		return $value;
	}	
				if($table=="Product Cost Breakdown" && $field=="ProdThaiName")
	{
		$value = $data["ProdThaiName"]." ".$data["ThaiModel"];
		return $value;
	}	
				if($table=="Product Cost Breakdown" && $field=="ProdEngName")
	{
		$value = $data["ProdEngName"]." ".$data["EngModel"];
		return $value;
	}	
